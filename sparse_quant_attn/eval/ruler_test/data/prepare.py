...
def prepare_data(
    ...
    try:
        ...
        print(f"Module data.{benchmark}.constants not found.")

    tasks_base = module.TASKS
    with open(os.path.join(curr_folder, f"../{benchmark}.yaml"), "r") as f:
    ...
    # Add templates
    assert model_template_type in Templates, print(
        f"{model_template_type} is not found in {Templates.keys()}"
    )
    print("Using model template:!!!!!!!!!!!!!!", model_template_type)
    ...
    chunks = [
        (num_samples // chunk_amount)
        + (1 if i < num_samples % chunk_amount else 0)
    ...
    try:
        script = os.path.join(curr_folder, benchmark, f"{config['task']}.py")
        ...
        {additional_args} \
        {f"--remove_newline_tab" if remove_newline_tab else ""} \
    ...
    save_file = save_dir / task / f"{subset}.jsonl"
    print(f"Prepare {task} with lines: {num_samples} to {save_file}")
